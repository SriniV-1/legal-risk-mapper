...
──────────────────────
Embedding-based risk detection that complements the ML classifier layer.
...
Pipeline:
  ...
  3. Compare each clause against auto-generated canonical embeddings
     (representative training examples selected by centroid distance)
  4. For each clause, emit risks for the top-K canonical matches above threshold
     (with de-duplication across risk categories)

The canonical clauses are auto-generated by the training pipeline — no
hand-curated entries. When both the ML classifier and semantic layer agree
on a risk, the merge pipeline applies an agreement bonus for higher confidence.

Fallbacks:
  • No canonical_clauses.json → semantic layer returns [] (run training first)
...
# Domain gates are no longer needed — the auto-generated canonical clauses
# are representative examples from training data, not domain-specific
# references (like HIPAA/FCPA). The ML classifier handles domain specificity.


def _passes_domain_gate(canonical_id: str, clause_text: str) -> bool:
    """Legacy interface — always passes since canonicals are auto-generated."""
    return True
